@props(['listing', 'badge' => null, 'badgeIcon' => 'la-star', 'badgeText' => null, 'badgeGradient' => 'linear-gradient(135deg, #FFB800, #FF8C00)'])
...
" onmouseover="this.style.transform='translateY(-8px)'; this.style.boxShadow='0 16px 48px rgba(0, 0, 0, 0.2)'; this.style.borderColor='rgba(255, 255, 255, 0.25)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 8px 32px rgba(0, 0, 0, 0.1)'; this.style.borderColor='rgba(255, 255, 255, 0.15)'">
    ...
    <div style="position: relative; overflow: hidden; height: 200px; background: linear-gradient(135deg, #2a3f4a, #1f2d35); flex-shrink: 0;">
        ...
        <!-- Badge -->
        @if($badge || $listing->is_featured == 1)
            <div style="
                position: absolute;
                top: 12px;
                left: 12px;
                ...
                gap: 6px;
                padding: 6px 14px;
                background: {{ $badge ? 'linear-gradient(135deg, #5b9bd5, #4a88c2)' : $badgeGradient }};
                color: white;
                border-radius: 20px;
                font-size: 12px;
                font-weight: 600;
            ...
            ">
                <i class="las {{ $badge ? 'la-star' : $badgeIcon }}"></i>
                <span>{{ $badge ? $badgeText ?? __('Badge') : __('En vedette') }}</span>
            </div>
        @endif
