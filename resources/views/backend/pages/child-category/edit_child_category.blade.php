            ...
            <div class="dashboard__card bg__white padding-20 radius-10">
                ...
                <form action="{{route('admin.child.category.edit',$child_category->id)}}" method="post" enctype="multipart/form-data" id="edit_category_form">
                    ...
                    <div class="form__input__flex">
                        ...
                        <div class="form__input__single">
    <label for="code" class="form__input__single__label"> {{ __('Code') }} <span class="text-danger">*</span> </label>
    <select name="code" id="code" class="select2_activation radius-5">
        <option value="0" {{ $child_category->code == 0 ? 'selected' : '' }}>{{ __('Selectionner le code') }}</option>
        <option value="1" {{ $child_category->code == 1 ? 'selected' : '' }}>Immobilier</option>
        <option value="2" {{ $child_category->code == 2 ? 'selected' : '' }}>Service</option>
        <option value="3" {{ $child_category->code == 3 ? 'selected' : '' }}>Occasion</option>
        <option value="4" {{ $child_category->code == 4 ? 'selected' : '' }}>Car</option>
    </select>
</div>
