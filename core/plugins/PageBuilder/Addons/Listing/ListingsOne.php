...
class ListingsOne extends PageBuilderBase
{
    ...
    public function frontend_render() : string
    {
        ...
        $search_placeholder = $settings["listing_search_by_text"] ??  __("Que recherchez vous?");

        $category_text = $settings["category"] ?? __("Choisir");
        $subcategory_text = $settings["subcategory"] ?? __("Choisir");
        $child_category_text = $settings["child_category"] ?? __("Choisir");
 ...
 $listings_top = Listing::where('status', 1)->where('is_published', 1);
        if (moduleExists('Membership') && membershipModuleExistsAndEnable('Membership')) {
            $listings_top = $listings_top->where(function ($query) {
                $query->whereHas('user_membership');
            });
        }

        $listings_top = $listings_top->orderBy('view', 'desc')
            ->take($items)
            ->where('status', 1)
        ...
        if (!empty(request()->get("country"))) {
           ...
           $listing_query->whereIn("state_id", $listings_country_ids)->get();
            $listings_top_country_ids = [];
            if ($listings_country) {
                $listings_top_country_ids = $listings_country->states->pluck("id")->toArray();
            }
           $listings_top->whereIn("state_id", $listings_top_country_ids)->get();
        }
        ...
        if (!empty(request()->get("cat"))) {
            ...
            $listings_top = $listings_top->where("category_id", request()->get("cat"));
        }

        if (!empty(request()->get("subcat"))) {
            $listing_query->where("sub_category_id", request()->get("subcat"));
             $listings_top = $listings_top->where("sub_category_id", request()->get("subcat")); 
        }
        ...
        if (!empty(request()->get("child_cat"))) {
           ...
           $listings_top = $listings_top->where(
                "child_category_id",
                request()->get("child_cat")
            );
        }
        ...
        // no remove
        $sortby_search = [
            "latest_listing" => __("Annonces récentes"),
        ...
        // add filter for check user has set his zone or not->where(function ($query) use ($memberIds){
