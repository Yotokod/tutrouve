...
if ($user_membership_check && ($user_membership_check->status === 0 || $user_membership_check->payment_status == 'pending')) {
    toastr_error(__('Votre plan d\'abonnement est inactif. Vous devez souscrire à un abonnement pour créer des annonces..'));
        ...
        if ($request->isMethod('post')) {
            ...
            if (get_static_option('listing_create_settings') == 'verified_user'){
                ...
                if($user_verified_status != 1 ){
                    toastr_error(__('Vous n\'êtes pas vérifié. Pour ajouter des annonces, vous devez d\'abord vérifier votre compte.'));
                    return redirect()->back();
                }
            }
            ...
            if(moduleExists('Membership')){
                if(membershipModuleExistsAndEnable('Membership')){
                    ...
                    if(is_null($user_membership)){
                        toastr_error(__('Vous devez souscrire à un abonnement pour créer des annonces.'));
                        return redirect()->back();
                    }
                    ...
                    if ($user_membership->listing_limit == 0 && $user_membership->expire_date <= Carbon::now()){
                        session()->flash('message', __('Votre abonnement a expiré.'));
                        return redirect()->back();
                    }elseif ($user_membership->listing_limit === 0){
                        toastr_error(__('Votre limite d\'annonces pour l\'abonnement est atteinte ! Veuillez le renouveler.'));
                        return redirect()->back();
                    }elseif ($user_membership->expire_date <= Carbon::now()){
                        toastr_error(__('Votre abonnement a expiré.'));
                        return redirect()->back();
                    }
                    ...
                    if ($gallery_images_input_count > $initial_gallery_images) {
                        toastr_error(__('Vous avez dépassé le nombre maximum d\'images de galerie autorisé par votre forfait d\'abonnement.'));
                        return redirect()->back();
                    }
                    ...
                    if (!empty($request->is_featured)){
                        if ($user_membership->initial_featured_listing != 0){
                            if ($user_membership->featured_listing === 0) {
                                toastr_error(__('Vous avez dépassé le nombre maximum d\'annonces en vedette autorisé par votre forfait d\'abonnement.'));
                                return redirect()->back();
                            }
                        }
                    }
    ...
    'title.required' => __('Le champ titre est requis.'),
    'title.max' => __('Le titre ne doit pas dépasser 191 caractères.'),
    'description.required' => __('Le champ description est requis.'),
    'description.min' => __('La description doit contenir au moins 150 caractères.'),
    'slug.required' => __('Le champ slug est requis.'),
    'slug.unique' => __('Ce slug est déjà pris.'),
    'price.required' => __('Le champ prix est requis.'),
    'price.numeric' => __('Le prix doit être une valeur numérique.')
]
);
            ...
            $listing->type_bien = $request->type_bien;
$listing->genre_bien = $request->genre_bien;
$listing->surface = $request->surface;
$listing->nbrs_piece = $request->nbrs_piece;
$listing->salle_bain = $request->salle_bain;
$listing->classe_energie = $request->classe_energie;
$listing->ges = $request->ges;
$listing->etage_bien = $request->etage_bien ?? 0; // Valeur par défaut 0 si null
$listing->etage_batiment = $request->etage_batiment ?? 0;
$listing->nature_bien = $request->nature_bien;
$listing->type_chambre = $request->type_chambre;
$listing->nbrs_colocataire = $request->nbrs_colocataire ?? 0;
$listing->nbrs_chambre = $request->nbrs_chambre ?? 0;
$listing->statut_fumeur = $request->statut_fumeur;
$listing->animaux = $request->animaux;
$listing->car_brand = $request->car_brand;
$listing->model = $request->model;
$listing->transmission = $request->transmission;
$listing->kilometer = $request->kilometer;
$listing->carburant = $request->carburant;
$listing->portiere = $request->portiere;
...
$listing->portiere = $request->portiere;
            ...
            try {
                ...
                Mail::to(get_static_option('site_global_email'))->send(new BasicMail([
                    'subject' => get_static_option('service_approve_subject') ?? __('Nouvelle demande d\'approbation d\'annonce'),
                    'message' => $message
                ]));
            } catch (\Exception $e) {
                //
            ...
            return back()->with(toastr_success(__('Annonce mise à jour avec succès')));
        }
    ...
    public function deleteListing($id = null)
    {
        if (Listing::find($id)) {
            ...
            toastr_error(__('Suppression de l\'annonce réussie---'));
            return redirect()->back();
        } else {
            toastr_error(__('Annonce non trouvée'));
            return redirect()->back();
        }
    }
    ...
    public function listingPublishedStatus($id)
    {
        ...
        if (!$listing) {
            $message = __('Annonce non trouvée.');
        ...
        if ($listing->status === 0) {
            $message = __('Cette annonce n\'est pas encore approuvée. Elle sera publiée après approbation.');
        ...
        if ($listing->is_published) {
            ...
            $message = __('L\'annonce a été publiée avec succès.');
            toastr()->success($message);
        } else {
            // Listing is unpublished
            $message = __('L\'annonce a été dépubliée avec succès.');
            toastr()->warning($message);
        }
