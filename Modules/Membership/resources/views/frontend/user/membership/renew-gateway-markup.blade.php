            ...
            <div class="modal-content">
                <div class="modal-header">
                    @if(Auth::guard('web')->check())
                       <h4>{{ get_static_option('membership_renew_modal_title') ?? __('Renouvéller l\'abonnement') }}</h4>
                    @else
                        <x-notice.general-notice :description="__('Avis : Veuillez vous connecter en tant qu\'utilisateur pour acheter un abonnement..')" />
                    @endif
                </div>
                ...
                <div class="modal-footer">
                    <div class="btn-wrapper">
                        <button type="button" class="btn-profile btn-outline-gray btn-hover-danger" data-bs-dismiss="modal">{{ __('Fermer') }}</button>
                        @if (Auth::guard('web')->check())
                            <button type="submit" class="btn-profile btn-bg-1 buy_membership" id="confirm_buy_membership_load_spinner">
                                {{ __('Payer maintenant') }} <span id="buy_membership_load_spinner"></span>
                            </button>
                        @endif
                    </div>
                </div>
